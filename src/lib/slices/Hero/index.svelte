<script lang="ts">
	...
	onMount(() => {
		...
		tl.fromTo('.hero__button', { scale: 1.5 }, { scale: 1, opacity: 1, duration: 1.2 }, '-=0.8');
		tl.fromTo('.hero__image', { y: 100 }, { y: 0, opacity: 1, duration: 1.2 }, '+=0.3');
		tl.fromTo('.hero__glow', { sclae: 0.5 }, { scale: 1, opacity: 1, duration: 1.2 }, '-=1');
...
<Bounded data-slice-type={slice.slice_type} data-slice-variation={slice.variation}>
	<div class="relative text-center">
		...
		{#if slice.primary.body}
			<p class="hero__body mx-auto mt-4 max-w-md text-balance text-gray-300 opacity-0">
				<PrismicText field={slice.primary.body} />
			</p>
		{/if}
		{#if slice.primary.button_link}
			<ButtonLink class="hero__button my-8 opacity-0" field={slice.primary.button_link}
		...
		{#if slice.primary.image}
			<div class="hero__image glass-container mt-16 w-fit opacity-0">
				<div
					class="hero__glow hero__glow-one absolute left-1/3 top-0 -z-10 h-2/3 w-2/3 bg-violet-700/50 mix-blend-screen blur-[80px] filter md:blur-[120px]"
				/>
				<div
					class="hero__glow hero__glow-two absolute left-0 top-1/3 -z-10 h-2/3 w-2/3 bg-orange-600/50 mix-blend-screen blur-[80px] filter md:blur-[120px]"
				/>
