...
<Bounded data-slice-type={slice.slice_type} data-slice-variation={slice.variation}>
	<h2 class="tex-center max-w-2xl text-balance text-5xl font-medium md:text-7xl">
		<PrismicText field={slice.primary.heading} />
	</h2>
	<div class="mx-auto mt-6 max-w-md text-balance text-center text-gray-300"></div>
	<PrismicRichText field={slice.primary.body} />

	<div class="mt-20 grid gap-16">
		<!-- loop over the array of case studies -->
		{#each caseStudies as caseStudy, index}
			...
			>
				...
				<div class={clsx('relative lg:col-span-2', index % 2 && 'md:-order-1')}>
					...
					<div class="image-glow -right-4 -top-8 bg-orange-500"></div>
					<PrismicImage
...
<style>
	.image-glow {
		@apply absolute h-1/2 w-1/2 rounded-full opacity-40 mix-blend-screen blur-3xl transition-opacity duration-300 group-hover:opacity-30;
	}
</style>
